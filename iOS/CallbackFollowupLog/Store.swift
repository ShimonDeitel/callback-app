...
final class Store: ObservableObject {
    ...
    private let fileName = "callback_entries.json"
    ...
    static func seedData() -> [Entry] {
        let now = Date()
        return [
            Entry(title: "Sample Callback One", detail: "First example entry.", date: now),
            Entry(title: "Sample Callback Two", detail: "Second example entry.", date: now.addingTimeInterval(-86400)),
            Entry(title: "Sample Callback Three", detail: "Third example entry.", date: now.addingTimeInterval(-172800)),
        ]
    }
}
